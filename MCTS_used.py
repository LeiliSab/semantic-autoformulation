    ...
    def rename_children(self, these_children, add_name):
        for child in these_children:
            try:
                ...
                # On Windows, file locks can prevent rename; skip with warning
                new_dir = modify_directory(child.this_dir, add_name)
                print(f"[WARNING] Could not rename {child.this_dir} to {new_dir}: {e}. Skipping.")
...
class MCTS:
    ...
    def __init__(
        ...
        num_warmup=3,
        use_sac=False,
        gamma=0.1,  # Weight for constraint uncertainty term in UCT
    ):
        self.Q = {}  # Accumulated rewards
        self.N = {}  # Visit counts
        self.Uc = {}  # Constraint uncertainty scores
        self.fR = {}
        ...
        self.gamma = gamma  # Uncertainty weight in UCT formula
        
        # Scoring strategies (set defaults, can be updated via update_global_scoring)
        self.global_scoring = "ranking"  # Options: "ranking" or "score"
        self.local_scoring = None

    def get_anchors_rec(self, node: ReasoningMCTSNode, n_samples=1):
    ...
    """
        ...
        # Extract formulation components
        objective = node.form_dict_eval.get('objective', {})
        eq_constraints = node.form_dict_eval.get('equality_constraints', {})
        ineq_constraints = node.form_dict_eval.get('inequality_constraints', {})
        parameters = node.form_dict_eval.get('parameters', {})
        decision_variables = node.form_dict_eval.get('decision_variables', {})
    
        # Load the executable code
        try:
            executable_code = node.load_python_script()
        except:
            executable_code = node.output_str
    
        # Reconstruct constraints and objectives
        reconstructed = {
            'objective': {},
            'equality_constraints': {},
            'inequality_constraints': {}
        ...
        # Process objective
        if objective:
            for obj_key, obj_desc in objective.items():
                prompt = prompt_constraints_language.format(
                    description=node.problem_str,
                    solver="Gurobi",
                    constraint=obj_desc,
                    constraint_code=self._extract_objective_code(executable_code),
                    params=str(parameters),
                    vars=str(decision_variables)
                )
            
                response = chat_gpt(user_prompt=prompt, n_used=n_samples, engine_used=self.engine_used)
                reconstructed_desc = self._parse_constraint_response(response.choices[0].message.content)
                reconstructed['objective'][obj_key] = reconstructed_desc
    
        # Process equality constraints
        for const_key, const_desc in eq_constraints.items():
            if const_desc is None or check_none_key_value({const_key: const_desc}):
                continue
            
            prompt = prompt_constraints_language.format(
                description=node.problem_str,
                solver="Gurobi",
                constraint=const_desc,
                constraint_code=self._extract_constraint_code(executable_code, const_key, 'equality'),
                params=str(parameters),
                vars=str(decision_variables)
            )
            ...
            response = chat_gpt(user_prompt=prompt, n_used=n_samples, engine_used=self.engine_used)
            reconstructed_desc = self._parse_constraint_response(response.choices[0].message.content)
            reconstructed['equality_constraints'][const_key] = reconstructed_desc
    
        # Process inequality constraints
        for const_key, const_desc in ineq_constraints.items():
            if const_desc is None or check_none_key_value({const_key: const_desc}):
                continue
            ...
            prompt = prompt_constraints_language.format(
                ...
                solver="Gurobi",
                constraint=const_desc,
                constraint_code=self._extract_constraint_code(executable_code, const_key, 'inequality'),
                params=str(parameters),
                vars=str(decision_variables)
            )
            ...
            response = chat_gpt(user_prompt=prompt, n_used=n_samples, engine_used=self.engine_used)
            reconstructed_desc = self._parse_constraint_response(response.choices[0].message.content)
            reconstructed['inequality_constraints'][const_key] = reconstructed_desc
    
        return reconstructed

    def _extract_objective_code(self, executable_code):
        """Extract objective function code from Gurobi script"""
        pattern = r'model\.setObjective\((.*?)\)'
        match = re.search(pattern, executable_code, re.DOTALL)
        return match.group(1) if match else ""
    
    def _extract_constraint_code(self, executable_code, const_key, const_type):
        """Extract specific constraint code from Gurobi script"""
        # Look for addConstr or addConstrs with constraint name
        pattern = rf'model\.addConstr[s]?\((.*?),\s*name\s*=\s*["\'].*?{re.escape(const_key)}.*?["\']\)'
        match = re.search(pattern, executable_code, re.DOTALL)
        return match.group(1) if match else ""

    def _parse_constraint_response(self, response_text):
        """Parse LLM response to extract constraint description"""
        pattern = r'CONSTRAINT:\s*=====\s*(.*?)\s*====='
        match = re.search(pattern, response_text, re.DOTALL)
        return match.group(1).strip() if match else response_text.strip()
    
    def get_anchors_ref(self, node: ReasoningMCTSNode):
        objective = node.form_dict_eval.get('objective', {})
        eq_constraints = node.form_dict_eval.get('equality_constraints', {})
        ineq_constraints = node.form_dict_eval.get('inequality_constraints', {})
        # Reconstruct constraints and objectives
    ...
    def update_collected_data(self, path, collected_rewards, collected_path, result_str, node=None):
        ...
        # Log SAC components when enabled and node is available (for eval analysis)
        if self.use_sac and node is not None:
            try:
                base_reward = self.get_reward_update_ranking(node)
                base_reward = 0 if base_reward is None else base_reward
            except Exception as e:
                print(f"SAC logging: base_reward error: {e}")
                base_reward = None

            try:
                sac_multiplier = self.get_SAC_score(node, n_eval=3)
            except Exception as e:
                print(f"SAC logging: sac_multiplier error: {e}")
                sac_multiplier = None

            if base_reward is not None and sac_multiplier is not None:
                new_dict["base_reward"] = base_reward
                new_dict["sac_multiplier"] = sac_multiplier
                new_dict["final_reward"] = base_reward * sac_multiplier
    ...
    def dfs(self, node, collected_path=[], collected_prior_reward=[]):
        ...
        if node.this_step == "inequality_constraints":
            self.update_collected_data(
                self.path,
                collected_prior_reward.copy(),
                collected_path.copy(),
                node.output_str,
                node=node if self.use_sac else None,
            )
            return
    ...
    def update_collected_result_rollout(self, node, final_reward=None, sac_multiplier=None, base_reward=None):
        to_update_dict = {}
        ...
        if sac_multiplier is not None:
            to_update_dict["sac_multiplier"] = sac_multiplier
        if base_reward is not None:
            to_update_dict["base_reward"] = base_reward
    ...
    def many_rollouts(self, path_dummy_node, total_rollouts, global_scoring="ranking", local_scoring=None, update_baseline=False):
        ...
        while True:
            ...
            if num_rollout == total_rollouts:
                break
            # Use the appropriate rollout function based on use_sac flag
            if self.use_sac:
                succesful_rollout = self.rollout_including_SAC(dummy_node, not_create_children=True)
            else:
    ...
    def rollout_including_SAC(self, node: MCTSNode, not_create_children=False):
        ...
        if path[-1] in self.r_baseline:
            ...
            # Compute base reward (Em_LLM component)
            if is_achieve_solution or self.global_scoring == "score":
                base_reward = self.get_reward_update_ranking(path[-1]) # LLM component of ranking returning float in [0,1]
                base_reward = 0 if base_reward is None else base_reward # ensuring base reward is a number
                
                # Compute SAC multiplier (semantic agreement between code and formulation)
                try:
                    sac_multiplier = self.get_SAC_score(path[-1], n_eval=3)
                except Exception as e:
                    print(f"SAC computation error: {e}")
                    sac_multiplier = 1.0
                
                # Final reward: I(solver) * Em_LLM * SAC
                reward = base_reward * sac_multiplier

                # Debug: trace SAC contribution when solver succeeds
                if is_achieve_solution:
                    print(f"[SAC DEBUG] node_idx={path[-1].idx} base_reward={base_reward} sac_multiplier={sac_multiplier} final_reward={reward}")
                
                # Warmup baseline handling with SAC multiplier
                if is_achieve_solution and (len(self.baselines_results) < self.num_warmup) and not (self.global_scoring == "score"):
                    ...
                    reward = 0.5 * sac_multiplier
            else:
                reward = 0
            
            self.r_baseline[path[-1]] = reward
            self.update_collected_result_rollout(
                path[-1], final_reward=reward, sac_multiplier=sac_multiplier, base_reward=base_reward
            )
            if self.update_baseline:
                self.update_baselines_results()
        
        # Compute constraint uncertainty for terminal node
        uncertainty = 0.0
        if path[-1].this_step == "inequality_constraints":
            try:
                uncertainty = self.compute_node_uncertainty(path[-1])
            except Exception as e:
                print(f"[WARNING] Could not compute uncertainty: {e}")
                uncertainty = 0.0

        self._back_propagate(path, reward=reward, uncertainty=uncertainty)
        return effective
    ...
    def _back_propagate(self, path, reward, uncertainty=None):
        """
        Backpropagate reward and uncertainty through the path.
        ...
        uncertainty : float, optional
            Constraint uncertainty to propagate (computed from leaf node)
        """
        for node in reversed(path):
            ...
            if uncertainty is not None:
                self.Uc[node] += uncertainty
            
            print("updating: ", node.this_step, " , idx", node.idx)

    def _uct(self, node: MCTSNode, log_n_f: float):
        """
        UCT selection formula augmented with constraint uncertainty.
        
        Formula: V(n) + ω*sqrt(ln(N)/N_child) + γ*Uc(n_child)
        """
        if node not in self.N:
            self.N[node] = 0
        if node not in self.Uc:
        ...
        if self.N[node] == 0:
            # First visit: use prior reward + exploration + uncertainty bonus
            exploration_term = self.w_exp * math.sqrt(log_n_f)
            uncertainty_term = self.gamma * self.Uc.get(node, 0)
            return node.r0 + exploration_term + uncertainty_term
        else:
            # Compute value term (combination of prior and empirical reward)
            value = self.lambda_cte * node.r0 + (1 - self.lambda_cte) * self.Q[node] / self.N[node]
            
            # Compute exploration term
            exploration_term = self.w_exp * math.sqrt(log_n_f / self.N[node])
            
            # Compute uncertainty term (average uncertainty from visits)
            avg_uncertainty = self.Uc[node] / self.N[node]
            uncertainty_term = self.gamma * avg_uncertainty
            
            return value + exploration_term + uncertainty_term
    ...
    # --- SAC (Semantic Agreement Check) helpers ---
    def _parse_yes_no_answer(self, text: str) -> str:
        """
        Extract YES/NO from an LLM response formatted as:
        ANSWER:
        =====
        YES or NO
        =====

        Falls back to a simple search if strict pattern not found.
        """
        try:
            pattern = r"ANSWER:\s*=+\s*(YES|NO)\s*=+"
            ...
            if "NO" in up and "YES" not in up:
                return "NO"
            # If both or none, default conservative to NO
            return "NO"
        except Exception:
            return "NO"

    def _compare_descriptions(self, ref_text: str, new_text: str, n_eval: int) -> float:
        """
        Compare two description strings and return YES vote fraction.
        ...
        n_eval : int
            Number of verifier samples
            
        Returns
        -------
        float
            Fraction of YES votes [0,1]
        """
        prompt_constraints_language_coverage = """
You are an expert in optimization modeling.

Your task is to judge the consistency of the new generated description and the original description of the same constraint.
...
"""
        ...
        prompt = prompt_constraints_language_coverage.format(
            constraint=ref_text,
            constraint_new=new_text,
        )
        
        yes_count = 0
        try:
            resp = chat_gpt(user_prompt=prompt, n_used=n_eval, engine_used=self.engine_used)
            for i in range(n_eval):
                content = resp.choices[i].message.content
                ans = self._parse_yes_no_answer(content)
                if ans == "YES":
                    yes_count += 1
        except Exception as e:
            print(f"Description comparison error: {e}")
            return 0.0
        
        return yes_count / max(1, n_eval)

    def get_SAC_score(self, node: ReasoningMCTSNode, n_eval: int = 3) -> float:
        
        # Get anchors internally
        ref = self.get_anchors_ref(node)
        rec = self.get_anchors_rec(node, n_samples=1)
        
        scores = []
        
        # Compare objective
        for k, rec_text in rec.get('objective', {}).items():
            ref_text = ref.get('objective', {}).get(k, None)
            if ref_text and rec_text:
                try:
                    s = self._compare_descriptions(str(ref_text), str(rec_text), n_eval)
                    scores.append(s)
                except Exception as e:
                    print(f"SAC objective error ({k}): {e}")
        
        # Compare equality constraints
        for k, rec_text in rec.get('equality_constraints', {}).items():
            ref_text = ref.get('equality_constraints', {}).get(k, None)
            if ref_text and rec_text:
                try:
                    s = self._compare_descriptions(str(ref_text), str(rec_text), n_eval)
                    scores.append(s)
                except Exception as e:
                    print(f"SAC eq constraint error ({k}): {e}")
        
        # Compare inequality constraints
        for k, rec_text in rec.get('inequality_constraints', {}).items():
            ref_text = ref.get('inequality_constraints', {}).get(k, None)
            if ref_text and rec_text:
                try:
                    s = self._compare_descriptions(str(ref_text), str(rec_text), n_eval)
                    scores.append(s)
                except Exception as e:
                    print(f"SAC ineq constraint error ({k}): {e}")
    ...
    def get_constraint_confidence(self, constraint_text: str, problem_description: str, constraint_type: str = "constraint") -> int:
        """
        Evaluate confidence level (1-5) for a single constraint using LLM.
        ...
        Returns
        -------
        int : Confidence score from 1 to 5
        """
        prompt = f"""
You are an expert in optimization modeling. Given the problem description below and a proposed {constraint_type} constraint, evaluate its necessity and correctness.
...
- Does this constraint logically make sense given the problem context?
- How confident are you that this constraint needs to be explicitly modeled in the optimization formulation?
- Rate your confidence on a scale from 1 (not confident, likely unnecessary) to 5 (very confident, essential).

At the end of your response, print "X OUT OF 5" where X is the confidence level (1, 2, 3, 4, or 5).
Do not generate anything after that line.
"""
        
        try:
            response = chat_gpt(user_prompt=prompt, n_used=1, engine_used=self.engine_used)
            content = response.choices[0].message.content
            
            # Parse "X OUT OF 5" pattern
            confidence_score = self._parse_confidence_score(content)
            return confidence_score
            
        except Exception as e:
            print(f"[WARNING] Constraint confidence error: {e}")
            return 3  # Default to neutral confidence
    
    def _parse_confidence_score(self, response_text: str) -> int:
        """
        Parse confidence score from LLM response.
        
        Expected format: "X OUT OF 5" where X is 1-5.
        """
        import re
        
        # Look for pattern "X OUT OF 5" (case insensitive)
        pattern = r'(\d)\s+OUT\s+OF\s+5'
        ...
        # Fallback: look for standalone numbers near "out of"
        pattern_fallback = r'(\d)[/\s]*out\s*of\s*5'
        match_fallback = re.search(pattern_fallback, response_text, re.IGNORECASE)
        
        if match_fallback:
            score = int(match_fallback.group(1))
            return max(1, min(5, score))
        
        print(f"[WARNING] Could not parse confidence score from: {response_text[-100:]}")
        return 3  # Default to neutral

    def _compute_constraint_entropy(self, confidence_score: int) -> float:
        """
        Compute entropy h(p) from confidence score (1-5).
        ...
        float : Entropy value
        """
        # Normalize to probability: p = (c - 1) / 4
        p = (confidence_score - 1) / 4.0
    ...
    def compute_node_uncertainty(self, node: ReasoningMCTSNode) -> float:
        """
        Compute node-level uncertainty Uc(n) as average of constraint entropies.
        ...
        Returns
        -------
        float : Node uncertainty score
        """
        # Only compute uncertainty for nodes with constraints
        if node.this_step not in ['equality_constraints', 'inequality_constraints']:
            return 0.0
        
        try:
            # Get confidence scores for all constraints
            confidence_dict = self.evaluate_constraints_confidence(node)
            
            entropies = []
            
            # Compute entropy for each constraint
            for constraint_type in ['equality_constraints', 'inequality_constraints']:
                for conf_score in confidence_dict[constraint_type].values():
                    entropy = self._compute_constraint_entropy(conf_score)
                    entropies.append(entropy)
            
            # Return average entropy (node-level uncertainty)
            if entropies:
            ...
            print(f"[WARNING] Error computing node uncertainty: {e}")
            return 0.0
    
    def evaluate_constraints_confidence(self, node: ReasoningMCTSNode) -> dict:
        """
        Evaluate confidence scores for all constraints in a node's formulation.
        
        Parameters
        ----------
        node : ReasoningMCTSNode
            Node containing constraint formulations
        
        Returns
        -------
        dict : Confidence scores for each constraint
            {
                'equality_constraints': {constraint_key: confidence_score, ...},
                'inequality_constraints': {constraint_key: confidence_score, ...}
            }
        """
        confidence_scores = {
            'equality_constraints': {},
        ...
        problem_desc = node.problem_str
        
        # Evaluate equality constraints
        eq_constraints = node.form_dict_eval.get('equality_constraints', {})
        for key, description in eq_constraints.items():
            if description and not check_none_key_value({key: description}):
                score = self.get_constraint_confidence(
                    str(description), 
                    problem_desc, 
                    constraint_type="equality"
                )
                confidence_scores['equality_constraints'][key] = score
        
        # Evaluate inequality constraints
        ineq_constraints = node.form_dict_eval.get('inequality_constraints', {})
        for key, description in ineq_constraints.items():
            if description and not check_none_key_value({key: description}):
                score = self.get_constraint_confidence(
                    str(description), 
                    problem_desc, 
                ...
                confidence_scores['inequality_constraints'][key] = score
