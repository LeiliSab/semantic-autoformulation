...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="The problem index (between 0 and 100).",
    )
    parser.add_argument(
        "--use-sac",
        action="store_true",
        help="Enable SAC (Semantic Agreement Check) multiplier in reward computation.",
    )
...
def _run_single_experiment(index: int, use_sac: bool = False) -> None:
    """
    ...
    use_sac : bool
        Whether to enable SAC multiplier in reward computation.
    """
    # Preserved path names
    this_dir = "NL4OPT"
    results_dir = "NL4OPT_results_SAC" if use_sac else "NL4OPT_results"
    ...
    this_mcts = MCTS(use_sac=use_sac)
...
_args = _parse_args()
_run_single_experiment(_args.index, use_sac=_args.use_sac)
